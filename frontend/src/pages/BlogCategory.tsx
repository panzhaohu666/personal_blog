...
export default function BlogCategoryPage() {
  ...
  if (isLoading) return <div className="flex min-h-screen items-center justify-center bg-[#F7F2E9] dark:bg-gray-900 text-[#8E8375]">加载中...</div>;
  if (isError) return <div className="flex min-h-screen items-center justify-center bg-[#F7F2E9] dark:bg-gray-900 text-red-500">加载失败: {String(error)}</div>;
        ...
        <aside className="w-full shrink-0 lg:w-60">
          <div className="rounded-xl border border-[#E9DFCE] dark:border-gray-700 bg-[#FFFDF8] dark:bg-gray-800 p-5">
            ...
            <div className="mt-3 flex flex-col gap-0.5">
              <Link to="/blog" className={linkClass(false)}>全部</Link>
              {categories?.map((cat) => (
                <Link key={cat.id} to={`/blog/category/${cat.slug}`} className={`flex justify-between ${linkClass(slug === cat.slug)}`}>
        ...
        <div className="min-w-0 flex-1">
          ...
          {data && data.items.length === 0 ? (
            <div className="rounded-xl border border-[#E9DFCE] dark:border-gray-700 bg-[#FFFDF8] dark:bg-gray-800 p-16 text-center">
              ...
              <Link to="/blog" className="mt-2 inline-block text-sm text-[#B9812F] dark:text-amber-400 hover:underline">返回首页</Link>
            </div>
          ) : (
