...
export default function AdminTags() {
  ...
  return (
    <div>
      <h1 className="mb-6 font-serif text-2xl font-bold text-[#2B2620] dark:text-gray-100">标签管理</h1>
      <form onSubmit={handleAdd} className="mb-4 flex items-end gap-2">
        <div className="flex flex-1 flex-col"><label className="block text-xs text-[#8E8375] dark:text-gray-400 mb-1">标签名称</label><input value={name} onChange={(e) => setName(e.target.value)} className="w-full rounded-xl border border-[#E9DFCE] dark:border-gray-700 bg-[#FFFDF8] dark:bg-gray-800 px-4 py-2 text-sm focus:border-[#B9812F] focus:outline-none" /></div>
      ...
      {isLoading ? <p className="text-[#8E8375] dark:text-gray-400">加载中...</p> : (
        <div className="overflow-x-auto rounded-xl border border-[#E9DFCE] dark:border-gray-700 bg-[#FFFDF8] dark:bg-gray-800">
          <table className="w-full text-left text-sm"><thead className="border-b border-[#E9DFCE] dark:border-gray-700 bg-[#F7F2E9] dark:bg-gray-900"><tr><th className="px-4 py-3 font-medium text-[#5F5649] dark:text-gray-300">名称</th><th className="px-4 py-3 font-medium text-[#5F5649] dark:text-gray-300">文章数</th><th className="px-4 py-3 font-medium text-[#5F5649] dark:text-gray-300">操作</th></tr></thead>
            <tbody>
              {tags?.length === 0 ? <tr><td colSpan={3} className="px-4 py-8 text-center text-[#8E8375] dark:text-gray-400">暂无标签</td></tr> : tags?.map((tag) => (
                <tr key={tag.id} className="border-b last:border-0"><td className="px-4 py-3 font-medium">{tag.name}</td><td className="px-4 py-3 text-[#8E8375] dark:text-gray-400">{tag.post_count}</td><td className="px-4 py-3"><button onClick={() => setConfirmId(tag.id)} className="text-red-500 hover:underline text-xs">删除</button></td></tr>
              ))}
            </tbody>
          </table>
        </div>
