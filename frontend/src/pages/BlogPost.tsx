...
export default function BlogPostPage() {
  ...
  if (!slug) return <div className="flex h-screen items-center justify-center bg-bg-warm dark:bg-gray-900 text-muted">无效的文章链接</div>;
  if (isLoading) return (
    <div className="min-h-screen bg-bg-warm dark:bg-gray-900 flex items-center justify-center">
  ...
  if (isError || !post) return (
    <div className="flex min-h-screen flex-col items-center justify-center bg-bg-warm dark:bg-gray-900 px-4 text-center">
      <h1 className="font-serif text-8xl font-bold text-gray-200 dark:text-gray-700">404</h1>
      <p className="mt-4 text-lg text-soft dark:text-gray-400">文章不存在或已删除</p>
    ...
    <div className="min-h-screen bg-bg-warm dark:bg-gray-900 text-primary-text dark:text-gray-100">
      ...
      <article className="mx-auto max-w-3xl px-4 py-10 sm:px-6">
        ...
        {post.tags && post.tags.length > 0 && (
          <div className="mt-10 flex flex-wrap gap-2 border-t border-line dark:border-gray-700 pt-6">
            {post.tags.map((tag) => (
